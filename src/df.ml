...
let empty columns =
  let rec loop : type a b c. (b, c) Column.columns -> a DataMap.data -> a DataMap.data = fun c acc -> match c with
    | [] -> acc
...
let v columns =
  let rec f_aux : type f c. t -> (f, t) Column.columns -> f = fun t v ->
    match v with
    | [] -> t
    | (Column.Int _ as v) :: tl -> fun x ->
      let (DataMap.Map acc) = t.data in
      let v = { data = DataMap.Map (DataMap.Cons ((v, Data.Int x), acc)) } in
      f_aux v tl
    | (Column.Float _ as v) :: tl -> fun x ->
      let (DataMap.Map acc) = t.data in
      let v = { data = DataMap.Map (DataMap.Cons ((v, Data.Float x), acc)) } in
      f_aux v tl
    | (Column.String _ as v) :: tl -> fun x ->
      let (DataMap.Map acc) = t.data in
      let v = { data = DataMap.Map (DataMap.Cons ((v, Data.String x), acc)) } in
      f_aux v tl
    | (Column.Bool _  as v) :: tl -> fun x ->
      let (DataMap.Map acc) = t.data in
      let v = { data = DataMap.Map (DataMap.Cons ((v, Data.Bool x), acc)) } in
      f_aux v tl
  in
...
let parse_csv_line s =
  ...
  Csv.of_string s |> Csv.input_all |> Csv.to_array |> Array.map Array.to_list |> fun v -> Array.get v 0

let headers f =
  let rec fold : type a b c. Column.t list -> (string * string) list -> Column.t list = fun acc -> function
    | [] -> List.rev acc
    | (column_name, v) :: rest ->
...
let data_of_array : type a. string array array -> int -> a Column.column -> a Data.data = fun arr idx -> function
  | Column.Int _ ->
    Data.Int (Array.map int_of_string arr.(idx))
  | Column.Float _ ->
    Data.Float (Array.map float_of_string arr.(idx))
  | Column.Bool _ ->
    Data.Bool (Array.map bool_of_string arr.(idx))
  | Column.String _ -> Data.String arr.(idx)

let rec column_exists : type a b c. int -> c Column.column -> Column.t list -> int option = fun idx c -> function
  | [] -> None
  | (Column.Hidden c') :: cs -> match Column.equal c c' with
...
let read_csv ?columns path =
  ...
  | Seq.Cons (header, rest) ->
    ...
    | Seq.Cons (first_line, rest) ->
      let zipped = List.combine (parse_csv_line header) (parse_csv_line first_line) in
      let headers = headers zipped in
      let headers_to_use =
        match columns with
        | Some columns ->
          let found, not_found =
            let rec idx : type a b. (Column.t * int option) list -> (a, b) Column.columns -> (Column.t * int option) list = fun acc -> function
              | [] -> List.rev acc
              | c :: cs ->
            ...
            let found = List.filter_map (function (c, Some v) -> Some (c, v) | _ -> None) is in
            let not_found = List.filter_map (function (c, None) -> Some c | (_, Some _) -> None) is in
            found, not_found
          in
          if not_found <> []
          then Fmt.failwith "Missing column(s) in the CSV: [%a]" Fmt.(list ~sep:(Fmt.any ", ") Column.pp) not_found
          else found
        | None -> List.mapi (fun i v -> (v, i)) headers
      in
      let all = first_line :: List.of_seq rest |> List.map parse_csv_line |> transpose in
